...
void display_life_med(life *l,  element *e, spidy_annimation *s)
{
    if (l->x == -200)
        l->valeur -= 1;
...
void annimation_bouffon_med(element *e, life *l, spidy_annimation *s)
{
...
void loop_game_med(life *l, element *e, sfSprite *back, spidy_annimation *s)
{
    sfFloatRect size_ball = sfSprite_getGlobalBounds(e->ball_bouffon);
    sfRenderWindow_drawSprite(e->win, back, NULL);
    shoot_ball_med(l, e, e->ball_bouffon);
    annimation_bouffon_med(e, l, s);
...
void gameplay_medium(element *e)
{
    ...
    while (sfRenderWindow_isOpen(e->win)) {
        while (sfRenderWindow_pollEvent(e->win, &e->event))
            ...
            : nothing();
        loop_game_med(l, e, background, s);
    }}
